...
export function Weather() {
    ...
    const WeaterBoxContainer = styled.div`
        ...
        .weatherFeelLike {
            font-size: 45px;
            line-height: 55px;
            color: linear-gradient(180deg, #FFFFFF 36.96%, #FFFFFF 36.97%, #DCDCDC 124.58%);
        }
        .weatherIcon {
            width: 79px;
            height: 87px;
    ...
    useEffect(async () => {
        ...
        async function fetchData() {
            ...
            .get(`?lat=${coords.lat}&lon=${coords.lon}&units=metric&lang=fr&appid=${import.meta.env.VITE_WEATHER_API_KEY}`)
            .then((response) => {
                ...
                <WeaterBoxContainer>
                    <div className="displayFlex">
                        <div className="displayFlex flexDirectionColumn">
                            <h2>{weather.name}</h2>
                            <span className="weatherDate">{new Date().toLocaleDateString("fr")}</span>
                        </div>
                        <Button></Button>
                    </div>
                    <div className="displayFlex">
                        <div className="weatherFeelLike">{parseInt(weather.main.feels_like)}<sup>°</sup>c</div>
